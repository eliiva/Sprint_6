...
class OrderPage(BasePage):
    ...
    def click_next_button(self):
        self.click_page_element(next_button)

    @allure.step('Дожидаемся отображения раздела Про аренду')
    def wait_for_load_rent_info_block(self):
        self.wait_for_load_element(complete_order_button)

    @allure.step('Указываем дату')
    def fill_date_input(self, date):
        self.fill_input(date_input, date)
        self.wait_for_load_element(date_selected)
        self.click_page_element(date_selected)

    @allure.step('Указываем срок аренды')
    def select_time_interval(self, interval_locator):
        self.click_page_element(time_interval_block)
        self.wait_for_load_element(time_interval_list)
        self.click_page_element(interval_locator)

    @allure.step('Выбираем цвет самоката')
    def select_scooter_color(self, color_locator):
        self.click_page_element(color_locator)

    @allure.step('Заполняем комментарий для курьера')
    def fill_comment_input(self, comment):
        self.fill_input(comment_input, comment)
